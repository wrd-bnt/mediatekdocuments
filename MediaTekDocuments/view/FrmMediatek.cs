...

{
    ...
    public partial class FrmMediatek : Form
    {
        ...
        private const string MSG_INFORMATION = "Information";
        ...
        private void btnReceptionExemplaireValider_Click(object sender, EventArgs e)
        {
            if (!txbReceptionExemplaireNumero.Text.Equals(""))
            {
                ...
                {
                    MessageBox.Show("le numéro de parution doit être numérique", MSG_INFORMATION);
                    txbReceptionExemplaireNumero.Text = "";
            ...
            {
                MessageBox.Show("numéro de parution obligatoire", MSG_INFORMATION);
            }
        }
        ...
        /// récupère les livres et les suivis, vide les zones
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void TabCommandesLivres_Enter(object sender, EventArgs e)
        {
            ...
            bdgSuivisLivres.DataSource = lesSuivis;
            cbxCommandesLivresSuivi.DataSource = bdgSuivisLivres;
        ...
        /// <summary>
        /// Affichage des informations du livre et de ses commandes
        /// </summary>
        /// <param name="livre">le livre concerné</param>
        private void AfficheCommandesLivresInfos(Livre livre)
        {
            ...
            lesCommandesLivres = lesCommandesLivres.OrderByDescending(o => o.DateCommande).ToList();
            RemplirCommandesLivresListe(lesCommandesLivres);
            grpCommandesLivresSaisie.Enabled = true;
            grpCommandesLivresSuivi.Enabled = false;
        }

        /// <summary>
        /// Vide les zones d'affichage des informations du livre et des commandes
        /// </summary>
        private void VideCommandesLivresInfos()
        {
            ...
            RemplirCommandesLivresListe(lesCommandesLivres);
            grpCommandesLivresSaisie.Enabled = false;
            grpCommandesLivresSuivi.Enabled = false;
        }

        /// <summary>
        /// Remplit le datagrid avec la liste des commandes recue en paramètre
        /// </summary>
        ...
        /// <summary>
        /// Sur la sélection d'une ligne dans le datagrid, affichage du suivi de la commande
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DgvCommandesLivresListe_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvCommandesLivresListe.CurrentCell != null)
            {
                try
                {
                    ...
                    cbxCommandesLivresSuivi.SelectedIndex = cbxCommandesLivresSuivi.FindStringExact(commande.Suivi);
                }
                catch
                {
                    grpCommandesLivresSuivi.Enabled = false;
        ...
        private void BtnCommandesLivresAjouter_Click(object sender, EventArgs e)
        {
            if (!txbCommandesLivresMontant.Text.Equals("") && !txbCommandesLivresNbExemplaires.Text.Equals(""))
            {
                try
                {
                    ...
                    int nbExemplaire = int.Parse(txbCommandesLivresNbExemplaires.Text);
                    ...
                    Suivi suiviEnCours = controller.GetAllSuivis().Find(x => x.Id.Equals("EC"));
                    CommandeDocument commande = new CommandeDocument(id, dateCommande, montant, nbExemplaire, idLivreDvd, "EC", suiviEnCours.Libelle);
                ...
                {
                    MessageBox.Show("le montant et le nombre d'exemplaires doivent être numériques", MSG_INFORMATION);
                }
            }
            else
            {
                MessageBox.Show("tous les champs doivent être remplis", MSG_INFORMATION);
            }
        }

        /// <summary>
        /// Modification de l'étape de suivi d'une commande en respectant les règles métier
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void BtnCommandesLivresSuiviModifier_Click(object sender, EventArgs e)
        {
            if (dgvCommandesLivresListe.SelectedRows.Count > 0)
            {
                CommandeDocument commande = (CommandeDocument)bdgCommandesLivresListe.List[bdgCommandesLivresListe.Position];
                Suivi suivi = (Suivi)cbxCommandesLivresSuivi.SelectedItem;
                if (suivi != null)
                {
                    if ((commande.IdSuivi.Equals("LI") || commande.IdSuivi.Equals("RE")) &&
                        (suivi.Id.Equals("EC") || suivi.Id.Equals("RL")))
                    {
                        MessageBox.Show("une commande livrée ou réglée ne peut pas revenir à une étape précédente", MSG_INFORMATION);
                        return;
                    }
                    if (suivi.Id.Equals("RE") && !commande.IdSuivi.Equals("LI"))
                    {
                        MessageBox.Show("une commande ne peut pas être réglée si elle n'est pas livrée", MSG_INFORMATION);
                        return;
                    }
                    commande.IdSuivi = suivi.Id;
                    if (controller.UpdateSuiviCommandeDocument(commande))
                    {
                        lesCommandesLivres = controller.GetCommandesDocument(txbCommandesLivresNumRecherche.Text);
                        lesCommandesLivres = lesCommandesLivres.OrderByDescending(o => o.DateCommande).ToList();
                        RemplirCommandesLivresListe(lesCommandesLivres);
                    }
                    else
                    {
                        MessageBox.Show("erreur lors de la modification du suivi", MSG_ERREUR);
                    }
                }
            }
            else
            {
                MessageBox.Show("une commande doit être selectionnée", MSG_INFORMATION);
            }
        }
        ...
        private void BtnCommandesLivresSupprimerCommande_Click(object sender, EventArgs e)
        {
            if (dgvCommandesLivresListe.SelectedRows.Count > 0)
            {
                ...
                if (commande.IdSuivi.Equals("LI") || commande.IdSuivi.Equals("RE"))
                {
                    MessageBox.Show("impossible de supprimer une commande déjà livrée ou réglée", MSG_INFORMATION);
                    return;
                }
                if (MessageBox.Show("voulez-vous vraiment supprimer cette commande ?", "Confirmation", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    {
                        MessageBox.Show("erreur lors de la suppression", MSG_ERREUR);
                    }
                }
            }
            else
            {
                MessageBox.Show("une commande doit être sélectionnée", MSG_INFORMATION);
            }
        }
        #endregion
        ...
        /// récupère les dvd et les suivis, vide les zones
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void TabCommandesDvd_Enter(object sender, EventArgs e)
        {
            lesDvd = controller.GetAllDvd();
            List<Suivi> lesSuivis = controller.GetAllSuivis();
            bdgSuivisDvd.DataSource = lesSuivis;
            cbxCommandesDvdSuivi.DataSource = bdgSuivisDvd;
        ...
        /// <summary>
        /// Affichage des informations du dvd et de ses commandes
        /// </summary>
        /// <param name="dvd">le dvd concerné</param>
        private void AfficheCommandesDvdInfos(Dvd dvd)
        {
            ...
            lesCommandesDvd = lesCommandesDvd.OrderByDescending(o => o.DateCommande).ToList();
            RemplirCommandesDvdListe(lesCommandesDvd);
            grpCommandesDvdSaisie.Enabled = true;
            grpCommandesDvdSuivi.Enabled = false;
        }

        /// <summary>
        /// Vide les zones d'affichage des informations du dvd et des commandes
        /// </summary>
        private void VideCommandesDvdInfos()
        {
            ...
            RemplirCommandesDvdListe(lesCommandesDvd);
            grpCommandesDvdSaisie.Enabled = false;
            grpCommandesDvdSuivi.Enabled = false;
        }
        ...
        private void DgvCommandesDvdListe_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvCommandesDvdListe.CurrentCell != null)
            {
                try
                {
                    ...
                    cbxCommandesDvdSuivi.SelectedIndex = cbxCommandesDvdSuivi.FindStringExact(commande.Suivi);
                }
                catch
                {
                    grpCommandesDvdSuivi.Enabled = false;
        ...
        private void BtnCommandesDvdAjouter_Click(object sender, EventArgs e)
        {
            if (!txbCommandesDvdMontant.Text.Equals("") && !txbCommandesDvdNbExemplaires.Text.Equals(""))
            {
                try
                {
                    ...
                    int nbExemplaire = int.Parse(txbCommandesDvdNbExemplaires.Text);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void BtnCommandesDvdSuiviModifier_Click(object sender, EventArgs e)
        {
            if (dgvCommandesDvdListe.SelectedRows.Count > 0)
            {
                CommandeDocument commande = (CommandeDocument)bdgCommandesDvdListe.List[bdgCommandesDvdListe.Position];
                Suivi suivi = (Suivi)cbxCommandesDvdSuivi.SelectedItem;
                if (suivi != null)
                {
                    if ((commande.IdSuivi.Equals("LI") || commande.IdSuivi.Equals("RE")) &&
                        (suivi.Id.Equals("EC") || suivi.Id.Equals("RL")))
                    ...
                    if (controller.UpdateSuiviCommandeDocument(commande))
                    {
                        lesCommandesDvd = controller.GetCommandesDocument(txbCommandesDvdNumRecherche.Text);
                        lesCommandesDvd = lesCommandesDvd.OrderByDescending(o => o.DateCommande).ToList();
                        RemplirCommandesDvdListe(lesCommandesDvd);
                    }
                    else
        ...
        /// récupère les revues et vide les zones
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        /// <summary>
        /// Affichage des informations de la revue et de ses abonnements
        /// </summary>
        ...
        private void AfficheCommandesRevuesInfos(Revue revue)
        {
            ...
            lesCommandesRevues = controller.GetAbonnementsRevue(revue.Id);
            lesCommandesRevues = lesCommandesRevues.OrderByDescending(o => o.DateCommande).ToList();
            RemplirCommandesRevuesListe(lesCommandesRevues);
            grpCommandesRevuesSaisie.Enabled = true;
            grpCommandesRevuesSuppression.Enabled = false;
        }

        /// <summary>
        /// Vide les zones d'affichage des informations de la revue et des abonnements
        /// </summary>
        private void VideCommandesRevuesInfos()
        {
            ...
            RemplirCommandesRevuesListe(lesCommandesRevues);
            grpCommandesRevuesSaisie.Enabled = false;
            grpCommandesRevuesSuppression.Enabled = false;
        }

        /// <summary>
        /// Remplit le datagrid avec la liste des abonnements reçue en paramètre
        /// </summary>
        ...
        /// <summary>
        /// Sur la sélection d'une ligne dans le datagrid, active la suppression
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void DgvCommandesRevuesListe_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvCommandesRevuesListe.CurrentCell != null)
            {
                try
                {
                    grpCommandesRevuesSuppression.Enabled = true;
                }
                catch
                {
                    grpCommandesRevuesSuppression.Enabled = false;
        ...
        private void BtnCommandesRevuesAjouter_Click(object sender, EventArgs e)
        {
            if (!txbCommandesRevuesMontant.Text.Equals(""))
            {
                try
                {
                    ...
                    {
                        lesCommandesRevues = controller.GetAbonnementsRevue(idRevue);
                        lesCommandesRevues = lesCommandesRevues.OrderByDescending(o => o.DateCommande).ToList();
                        RemplirCommandesRevuesListe(lesCommandesRevues);
                ...
                {
                    MessageBox.Show("le montant doit être numérique", MSG_INFORMATION);
                }
            }
            else
            {
                MessageBox.Show("le montant doit être renseigné", MSG_INFORMATION);
            }
        }

        /// <summary>
        /// Suppression d'un abonnement uniquement si aucun exemplaire n'est rattaché
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void BtnCommandesRevuesSupprimerCommande_Click(object sender, EventArgs e)
        {
            if (dgvCommandesRevuesListe.SelectedRows.Count > 0)
            {
                ...
                bool aDesExemplaires = lesExemplairesRevue.Exists(ex =>
                    Exemplaire.ParutionDansAbonnement(abonnement.DateCommande, abonnement.DateFinAbonnement, ex.DateAchat));
                if (aDesExemplaires)
                {
                    MessageBox.Show("impossible de supprimer un abonnement avec des exemplaires rattachés", MSG_INFORMATION);
                    return;
                }
                if (MessageBox.Show("voulez-vous vraiment supprimer cet abonnement ?", "Confirmation", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    {
                        lesCommandesRevues = controller.GetAbonnementsRevue(txbCommandesRevuesNumRecherche.Text);
                        lesCommandesRevues = lesCommandesRevues.OrderByDescending(o => o.DateCommande).ToList();
                        RemplirCommandesRevuesListe(lesCommandesRevues);
                    }
                    else
            ...
            {
                MessageBox.Show("un abonnement doit être sélectionné", MSG_INFORMATION);
            }
        }
        #endregion
